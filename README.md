...
一个基于 Python 和 PyQt5 开发的**高效屏幕坐标快速选择与点击工具**。通过全屏字符网格覆盖，让用户只需输入简短的字符组合（如 "
A1", "Z9"），即可精准定位并自动点击屏幕任意位置。
...
确保项目根目录下存在 `resource` 文件夹，并包含名为 `pandaFace.ico` 的图标文件。如果没有，可以自行替换为任意 `.ico`
或 `.png` 文件，并修改代码中的路径。
...
- [ ] 优化遮罩层的显示样式，从按钮变成纯文字排版
- [ ] 支持右键点击和双击操作
- [ ] 增加配置文件支持 (JSON/YAML)
- [ ] 优化高 DPI 屏幕适配
- [ ] 添加拖拽功能
- [ ] 全局钩子（Global Hook）与焦点管理问题 : 在编辑页面，弹出遮罩层后，输入，原先在的编辑器界面也会同步输入
   遮罩层窗口成功“抢”到焦点（Focus），底层编辑器自然就会失去焦点，输入的字符就只会进入遮罩层，不会泄露到底层。

<div style="text-align: center;">

**Made with ❤️ by [GHengS]**
